...
/*   Updated: 2022/05/16 13:21:34 by hdrabi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ClapTrap.hpp"

void gameplay(ClapTrap C[5])
{
    int clap;
    int action;
    int target;
    ...
    while (!std::cin.fail())
    {
        ...
        std::cout << "choose your ClapTrap (-1 to exit): ";
        std::cin >> clap;
        if (clap == -1)
        {
...
int main()
{
    ...
    std::cout << "Creating 5 ClapTraps" << std::endl;
    for (int i = 0; i < 5 ; i++)
    {
        std::cout << "give ClapTrap " << (i + 1) << " a name : ";
        std::cin >> name;
        C[i] = ClapTrap(name);
    }
    std::cout << "Editting ClapTraps" << std::endl;
    for (int i = 0; i < 5 ; i++)
    {
        std::cout << "\033[1m\033[33m" << "modify ClapTrap " << (i + 1) << " HP (-1 to keep default) : [" << C[i].getHP() << "] " << "\033[0m";
        std::cin >> value;
        if (value >= 0)
            C[i].setHP(value);
        std::cout << "\033[1m\033[33m" << "modify ClapTrap " << (i + 1) << " EP (-1 to keep default) : [" << C[i].getEP() << "] " << "\033[0m";
        std::cin >> value;
        if (value >= 0)
            C[i].setEP(value);
        std::cout << "\033[1m\033[33m" << "modify ClapTrap " << (i + 1) << " AP (-1 to keep default) : [" << C[i].getAD() << "] " << "\033[0m";
        std::cin >> value;
        if (value >= 0)
            C[i].setAD(value);
    }
