...
/*   Updated: 2022/05/16 13:21:57 by hdrabi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void gameplay(FragTrap C[5])
{
    ...
    while (!std::cin.fail())
    {
        ...
        std::cout << "choose your FragTrap (-1 to exit): ";
        std::cin >> clap;
        if (clap == -1)
        {
...
int main()
{
    ...
    std::cout << "Creating 5 FragTraps" << std::endl;
    for (int i = 0; i < 5 ; i++)
    {
        std::cout << "give FragTrap " << (i + 1) << " a name : ";
        std::cin >> name;
        C[i] = FragTrap(name);
    }
    std::cout << "Editting FragTraps" << std::endl;
    for (int i = 0; i < 5 ; i++)
    {
        std::cout << "\033[1m\033[33m" << "modify FragTrap " << (i + 1) << " HP (-1 to keep default) : [" << C[i].getHP() << "] " << "\033[0m";
        std::cin >> value;
        if (value >= 0)
            C[i].setHP(value);
        std::cout << "\033[1m\033[33m" << "modify FragTrap " << (i + 1) << " EP (-1 to keep default) : [" << C[i].getEP() << "] " << "\033[0m";
        std::cin >> value;
        if (value >= 0)
            C[i].setEP(value);
        std::cout << "\033[1m\033[33m" << "modify FragTrap " << (i + 1) << " AP (-1 to keep default) : [" << C[i].getAD() << "] " << "\033[0m";
        std::cin >> value;
        if (value >= 0)
            C[i].setAD(value);
    }
